у меня есть аддон https://www.curseforge.com/wow/addons/minimap-alert но мне нужно дополнить  функционал а именно после того как аддон minimap-alert найдет  руду на миникарте тогда я получаю координаты с миникарты где распологается нужная мне руда и эти координаты передаются в tomtom  то есть идет запуск tomtom c этими координатами. 
        ...
        if line then
            for _, node in pairs(minimapAlert.saveData.trackingList) do
                ...
                for w in string.gmatch(nodeName, ".+") do
                    if string.find(line, string.lower(w), 1, true) and not string.find(line, string.lower(w..'|'), 1, true) then --Temporary hack for Gathermate2
...
--Dragon flight quick an dirty, maybe a better way becomes available, store collected mounts instead of requesting every time
local function is_dragon_riding()
    if IsMounted() and C_MountJournal and C_MountJournal.GetCollectedDragonridingMounts then
        for k,v in pairs(C_MountJournal.GetCollectedDragonridingMounts()) do
            if select(4, C_MountJournal.GetMountInfoByID(v)) then
                return true
            end
        end
    end
